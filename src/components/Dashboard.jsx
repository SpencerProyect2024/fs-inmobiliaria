...
const Dashboard = ({ user, onLogout }) => {
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      // Intentamos enviar a ambos jefes a través del servidor
      const response = await api.post('/whatsapp/notificar', {
        cliente: formData.nombre_completo,
        telefono_prospecto: formData.telefono,
        proyecto: formData.proyecto,
        lote: formData.lote,
        fecha_cita: formData.cita_agendada,
        jefes: [NUMERO_JEFE_1, NUMERO_JEFE_2] // Lista de destinatarios
      });

      if (response.data.success) {
        setMessage('✅ Notificación enviada a ambos jefes correctamente');
      } else {
        // Respaldo manual (solo abre el chat del Jefe 1)
        const mensaje = 
            `*🔔 NUEVA CITA AGENDADA*\n\n` +
            `👤 *Cliente:* ${formData.nombre_completo}\n` +
            `📞 *Teléfono:* ${formData.telefono}\n` +
            `🏗️ *Proyecto:* ${formData.proyecto}\n` +
            `📍 *Lote:* ${formData.lote || 'N/A'}\n` +
            `📅 *Fecha y Hora:* ${formData.cita_agendada || 'Pendiente'}\n\n` +
            `_Confirmar disponibilidad._`;

        const url = `https://wa.me/${NUMERO_JEFE_1}?text=${encodeURIComponent(mensaje)}`;
        window.open(url, '_blank');
        setMessage('✓ Ventana de respaldo abierta con Jefe 1');
      }
    } catch (error) {
      setMessage('✗ Error al conectar con el servidor de WhatsApp');
    }
  };
  ...
  const inputStyle = { padding: '12px 16px', border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', boxSizing: 'border-box' }
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={simulatorStyle}>
              ...
              <div style={simulatorGridStyle}>
                <div style={formGroupStyle}><label style={labelStyle}>Precio Total *</label><input type="number" style={inputStyle} value={formData.precio_total} onChange={(e) => handleInputChange('precio_total', e.target.value)} /></div>
                <div style={formGroupStyle}><label style={labelStyle}>Separación</label><input type="number" style={inputStyle} value={formData.enganche_porcentaje} onChange={(e) => handleInputChange('enganche_porcentaje', e.target.value)} /></div>
                <div style={formGroupStyle}><label style={labelStyle}>Cuota Promesa</label><input type="number" style={inputStyle} value={formData.tasa_interes} onChange={(e) => handleInputChange('tasa_interes', e.target.value)} /></div>
                <div style={{ backgroundColor: '#24303c', padding: '16px', borderRadius: '8px', color: '#fff' }}>
            ...
            <div style={buttonsStyle}>
              <button type="submit" style={buttonBlueStyle} disabled={loading}><Save size={18} /> {editingId ? 'ACTUALIZAR' : 'GUARDAR EN BD'}</button>
              <button type="button" style={buttonWhiteStyle} onClick={handleGeneratePDF}><FileText size={18} /> GENERAR PDF</button>
              <button type="button" style={buttonGreenStyle} onClick={handleSendWhatsApp} disabled={loading}>
                <MessageCircle size={18} /> {loading ? 'PROCESANDO...' : 'NOTIFICAR JEFES'}
              </button>
            </div>
          </form>
                  ...
                  {lotes.map(lote => (
                    <tr key={lote.id} style={{borderBottom: '1px solid #ddd'}}>
                      ...
                      <td style={{padding: '12px'}}>
                        <button onClick={() => handleEdit(lote)} style={{marginRight: '8px', padding: '4px 8px', backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: '4px'}}>Editar</button>
                        <button onClick={() => handleDelete(lote.id)} style={{padding: '4px 8px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px'}}>Borrar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
